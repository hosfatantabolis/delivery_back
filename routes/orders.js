  ...
  try {
    ...
    if (req.user.role === 'manager') {
      ...
      if (deliveryPeriodStart || deliveryPeriodEnd) {
        query.deliveryDate = {};
        if (deliveryPeriodStart)
          query.deliveryDate.$gte = new Date(deliveryPeriodStart);
        if (deliveryPeriodEnd)
          query.deliveryDate.$lte = new Date(deliveryPeriodEnd);
      }
      if (date) {
    ...
    if (io) {
      ...
      if (assignedDriverId) {
        const notification = {
          ...
          message: `Вам назначен заказ ${orderNumber} для ${client.name}`,
          orderId: order._id,
          orderNumber: orderNumber,
        ...
        io.to(`user_${assignedDriverId.toString()}`).emit(
          'notification',
          notification,
        );
        io.to(`user_${assignedDriverId.toString()}`).emit(
  ...
  async (req, res) => {
    try {
      ...
      const notification = {
        ...
        type: 'assignment',
        title: '📋 Назначен новый заказ',
        message: `Вам назначен заказ ${updatedOrder.orderNumber} для ${updatedOrder.client?.name}`,
      ...
      if (io) {
        ...
        io.to(`user_${driverId.toString()}`).emit(
          'order-assigned',
          updatedOrder,
        );
        ...
        io.to('role_admin').emit('notification', {
          ...notification,
          message: `Заказ ${updatedOrder.orderNumber} назначен водителю ${driver.name}`,
        });
        ...
        if (rooms.has(driverRoom)) {
          console.log(
            `✅ Driver room ${driverRoom} exists, has ${rooms.get(driverRoom).size} clients`,
          );
        } else {
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    if (req.user.role === 'driver') {
      const orderDriverId = order.assignedDriver
        ? order.assignedDriver._id.toString()
        : null;
      ...
      if (!orderDriverId || orderDriverId !== currentUserId) {
        return res
          .status(403)
          .json({ error: 'You can only update your own assigned orders' });
      }
      ...
      if (!status) {
        return res
          .status(400)
          .json({ error: 'Drivers can only update order status' });
      }
      ...
      const notification = {
        ...
        title: `Обновление статуса заказа ${updatedOrder.orderNumber}`,
        message: `🚚 Водитель ${req.user.name} ${status === 'delivered' ? 'закончил заказ' : 'обновил'} статус заказа ${updatedOrder.orderNumber} на: ${status}`,
        orderId: order._id,
      ...
      if (io) {
        ...
        if (order.createdBy) {
          io.to(`user_${order.createdBy._id.toString()}`).emit(
            'notification',
            notification,
          );
        }
...
router.patch('/:id/status', auth, async (req, res) => {
  try {
    ...
    if (req.user.role !== 'driver') {
      return res
        .status(403)
        .json({ error: 'Only drivers can use this endpoint' });
    }
    ...
    if (
      ...
      order.assignedDriver.toString() !== req.userId.toString()
    ) {
      return res
        .status(403)
    ...
    const notification = {
      ...
      type: status === 'delivered' ? 'success' : 'info',
      title: `Обновление статуса заказа ${updatedOrder.orderNumber}`,
      message: `🚚 Водитель ${req.user.name} ${status === 'delivered' ? 'завершил' : 'обновил'} статус заказа ${updatedOrder.orderNumber} на: ${status}`,
      orderId: order._id,
      orderNumber: order.orderNumber,
    ...
    if (io) {
      ...
      if (order.createdBy) {
        io.to(`user_${order.createdBy.toString()}`).emit(
          'notification',
          notification,
        );
      }
...
router.put('/:id/status', auth, async (req, res) => {
  ...
  if (req.user.role !== 'driver') {
    return res
      .status(403)
      .json({ error: 'Only drivers can update order status' });
  }
  ...
  try {
    ...
    const notification = {
      ...
      type: status === 'delivered' ? 'success' : 'info',
      title: `Обновление статуса заказа ${updatedOrder.orderNumber}`,
      message: `🚚 Водитель ${req.user.name} ${status === 'delivered' ? 'завершил доставку' : 'обновил статус заказа'} на: ${status}`,
      orderId: order._id,
      orderNumber: order.orderNumber,
      timestamp: new Date().toISOString(),
      read: false,
